...
class puyo_puyo:
    ...
    def __init__(self,screen,renderOnEpisodeCount=1,episode=1,currentBoardQueue=[],currentPieceQueue=[],nextPieceQueue=[],movementQueue=[],resultQueue=[]):
        ...
        self.currentBoardQueue = currentBoardQueue
        self.currentPieceQueue = currentPieceQueue
        ...
        self.movementsDict= {
            'drop': self.drop,
            'left': self.moveleftAi,
            'right': self.moverightAi,
            'rotate': self.rotate,
        }
    ...
    def rungame(self,pos ="Null"):
        ...
        self.currentBoardQueue.put(self.board)
        self.currentPieceQueue.put(self.currentpuyoblock)
        self.nextPieceQueue.put(self.nextpuyoblock)
        ...
        while running:
            ...
            if not self.movementQueue.empty():
                self.movementsDict[self.movementQueue.get()]()
            self.held_down_movement()
            self.naturalmovement()
            
            #drawing it all on
            if self.episodeNumber%self.renderOnEpisodeCount==0:
                if pos == "Null":
    ...
    def scoreLines(self,pos):
        ...
        if values != 0:
            ...
            lhs = (10 * values[2])
            self.score += lhs + rhs
            self.resultQueue.put((self.score,values[1]))
            return lhs + rhs
        self.resultQueue.put(((self.score,0)))
        return 0
    ...
    def chaincheckandremove(self,block,pos):
        ...
        if firstflag or secondflag:
            ...
            if firstflag and secondflag:
                ...
                if pos == "Null" and self.episodeNumber%self.renderOnEpisodeCount==0:
                    self.draw_all_on_screen(pos)
    ...
    def currentpuyoblockcheck(self):
        if self.currentpuyoblock == None:
