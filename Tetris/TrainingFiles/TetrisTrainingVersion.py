...
class Tetris:
    ...
    def goDown(self):
        ...
        if self.currentBlock is not None and not self.gameOver:
            ...
            if self.intersects():
                self.currentBlock.y-=1
                amountOfLinesCleared = self.placeBlock()
        #return amount,self.score,self.level
    ...
    def goDrop(self):
        ...
        if self.currentBlock is not None:
            ...
            self.currentBlock.y-=1
            self.placeBlock()
            #return self.placeBlock(),self.score,self.level
    ...
    def scoreLines(self):
        ...
        if self.linesCleared>=10+(10*self.level):
            self.level+=1
        if score==1:
        ...
        #Here we append the results of the Ai's action onto another queue so it can access this information
        self.resultsQueue.put((score,self.score,self.level))
